...
\end{lstlisting}
%
Typically creation of this file will be automated and added to the shell script used to invoke the simulation, so ensure that process counts passed in throught the configuration file are valid. Alternative specification mechanisms such as ratio specification (equivalently percentage specification) are being considered for future use to prevent the possbility of misallocation of processes (the user not specifying assignment for all processes available to the simulation).

The assignment of processes to a process set is currently static, and processes cannot be reassigned during a simulation. Additionally the assignment of a scale to a process set is static as well. However, scale-tasks are capable of being dynamically created and destroyed during runtime. Additionally, scale-tasks associated with specific subsets of a process set (either a single process or several processes) may be migrated in part or in whole. Migrating a scale-task in part amounts to using load balancing facilities on the associated processes to redistribute all data scale-task from a scale-task to a subset of the previously associated processes, allowing the freed processes to be assigned data from other scale-tasks. Migrating a scale task in whole constitutes using load balancing operations on the process set associated with the entire scale to move all pertinent data between subset of the managed process set. In the current implementation these operations are both collective on the process set associated with the managed scale.
...
The biotissue simulation was run on the AMOS BG/Q machine housed at the CCI facilities. Both strong and weak scaling studies were conducted up to an eighth of the total machine core count, or 16384 cores. The ratio of macro-scale to micro-scale assigned processes was held near 1:128, though the total core count was kept as an integer power of 2 in order for the SLURM job scheduler to submit the jobs to the compute nodes in a timely manner. The precise calculation used to determine macro and micro process counts is given by 
%
\begin{lstlisting}[language=Bash,caption={process assignment calculation}]
  MICRO_RATIO=128
  NP=$(( $NUM_MACRO * $MICRO_RATIO))
