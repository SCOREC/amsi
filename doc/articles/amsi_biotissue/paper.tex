...
[Need a couple sentences saying that multi-scale is important]



Multi-scale influences can also be incorporated in a more strictly numerical or computational sense, combining implementations of established physical models operating at different scales referencing and effecting shared physical fields and values \cite{shenoy97} \cite{weinan2003heterogenous}. 
...
For either case the subset of the engineering-scale domain associated with finer scales are areas of the domain where the fine-scale contributions have the greatest impact on the engineering scale simulation, in order to leverage the most utility from the multi-scale approach.
...
Numerical multi-scale simulations fall broadly into two categories.
...
All Scales have a set of MPI_Ranks associated with them through an intermediary structure called the RankSet. 
