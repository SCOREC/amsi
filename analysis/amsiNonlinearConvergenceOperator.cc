...
namespace amsi
{
  ...
  {
    protected:
    ScalarFunctionMT<1> eps;
    public:
    MTUpdatingEpsilon(ScalarFunctionMT<1> e)
        : eps(std::move(e))
    { }
  ...
  std::unique_ptr<Convergence> createConvergenceOperator(const mt::AssociatedCategoryNode& nd,
                                                         ...
                                                         LAS * las)
  {
    if(nd.GetName() == "linear convergence")
    {
      return std::make_unique<LinearConvergence>();
    }
    if(nd.GetName() == "nonlinear iteration")
    {
      auto* cvg_tp = MTCast<IntMT>(nd.FindModelTrait("convergence type"));
      auto* ref_tp = MTCast<IntMT>(nd.FindModelTrait("reference value"));
      auto* eps_att = nd.FindModelTrait("epsilon");
      ...
      // cap_att is optional
      auto* cap_att = MTCast<IntMT>(nd.FindModelTrait("iteration cap"));
      if(cap_att)
      {
      ...
      std::cerr<<"epsilon must be either a scalar, or scalar function with one scalar input";
      exit(1);
    }
    return nullptr;
  }
